...
float JelloComplex1::centerX;
JelloComplex1::JelloComplex1(){
	AllocateShiftMem();
}
...
/* *** Test constructor ***    */
JelloComplex1::JelloComplex1(vector<Point2f> corners1, vector<Point2f> corners2, int length){
...
void JelloComplex1::AllocateShiftMem(){
...
void JelloComplex1::CalculateModelParameters(vector<Point2f> corners1, vector<Point2f> corners2, int length, float* &params){
	float * updatesModelLevel2 = new float[6];
	Model2NewtonIterationLS(corners2, corners1, length, params, updatesModelLevel2);
	...
	for(int frequencyNum = 0; frequencyNum < NUM_FREQUENCIES; frequencyNum++){
		...
		GetSineEstimatesWeighted(ys, diffs, weights, roundParams);
	...
	for(int i=0;i<steps;i++){
		...
		//printf("w: %f new LS cost: %f   welsch(4) cost: %f\n", w, FullModelCostLs(corners1, corners2, params), FullModelCostWelsch(corners1, corners2, endW, params));
	}
}

void JelloComplex1::FullModelWelschFit(vector<Point2f> corners1, vector<Point2f> corners2, int length, FLOAT_TYPE* params, FLOAT_TYPE* &updates, FLOAT_TYPE &lambda, FLOAT_TYPE w){
...
float JelloComplex1::FullModelCostWelsch(vector<Point2f> corners1, vector<Point2f> corners2, float w, float* params){
	float result = 0;
...
float JelloComplex1::FullModelCostLs(vector<Point2f> corners1, vector<Point2f> corners2, float* params){
	float result = 0;
	for(int i=0;i<(int)corners1.size();i++){
		float x, y, x2, y2;
		x = corners1[i].x;
...
#define F_HIGH	0.1
#define F_STEPS	20
void JelloComplex1::GetSineEstimatesWeighted(vector<float> ys, vector<float> diffs, vector<float> weights, float* &result){
	...
	for(int i=0;i<5;i++){
		ImproveSineEstimates(ys, diffs, weights, (int)ys.size(), result, updates, lambda);
		for(int j=0;j<3;j++){
			result[j] -= updates[j];
		}
	}
...
float JelloComplex1::SineEstimatesCost(vector<float> ys, vector<float> diffs, vector<float> weights, float* params){
	float result = 0;
	...
	for(int i=0;i<(int)ys.size();i++){
		...
		result += (pred-diffs[i]) * (pred-diffs[i]);
	}
...
void JelloComplex1::ImproveSineEstimates(vector<float> ys, vector<float> diffs, vector<float> weights, int length, float* params, float* &updates, float &lambda){
	...
	float startCost = SineEstimatesCost(ys, diffs, weights, params);
	...
	for(int i=0;i<length;i++){
		...
		FLOAT_TYPE t3 =  a*t2;
		FLOAT_TYPE t4 =  t3*w;
		FLOAT_TYPE t5 =  cos(t1);
		FLOAT_TYPE t6 =  -t4;
		FLOAT_TYPE t7 =  d + t6;
		FLOAT_TYPE t8 =  (w*w);
		FLOAT_TYPE t9 =  2*t8;
		FLOAT_TYPE t10 =  t7*w;
		FLOAT_TYPE t11 =  t10*t5;
		FLOAT_TYPE t12 =  (a*a)*(t5*t5);
		FLOAT_TYPE t13 =  t9*y;
		FLOAT_TYPE t14 =  2*t4;
		FLOAT_TYPE t15 =  t3*t5;

		jacob(0, 0) +=  (t2*t2)*t9;
		jacob(1, 0) +=  -2*t11*y + t13*t15;
		jacob(1, 1) +=  (y*y)*(t12*t9 + t14*t7);
		jacob(2, 0) +=  -2*t11 + t15*t9;
		jacob(2, 1) +=  t12*t13 + t14*t7*y;
		jacob(2, 2) +=  t12*t9 + t14*t7;
...
void JelloComplex1::Model2NewtonIterationLS(vector<Point2f> corners1, vector<Point2f> corners2, int length, FLOAT_TYPE* params, FLOAT_TYPE* &updates){
	...
	for(int i=0;i<length;i++){
		...
		FLOAT_TYPE t8 =  2*t1;
		FLOAT_TYPE t9 =  t1*x1;
		FLOAT_TYPE t10 =  t7*y1;
		FLOAT_TYPE t11 =  t3*t5 + t4*t6;
		FLOAT_TYPE t12 =  t3*x1;
		FLOAT_TYPE t13 =  t6*y1;
		FLOAT_TYPE t14 =  t1*y1;
		FLOAT_TYPE t15 =  dy0 + dy1*y1 + t14 + t2*x1 - y2;
		FLOAT_TYPE t16 =  dx0 + dx1*y1 + t10 + t9 - x2;
		FLOAT_TYPE t17 =  t12 + t13;
		FLOAT_TYPE t18 =  t8*x1;
		FLOAT_TYPE t19 =  t3*y1;
		FLOAT_TYPE t20 =  -t1;
		FLOAT_TYPE t21 =  t18 + t19;
		FLOAT_TYPE t22 =  t3*t4 + t5*t8;
		FLOAT_TYPE t23 =  2*t2;
		FLOAT_TYPE t24 =  2*y1;

			jacob(0, 0) +=  t15*t17 + t16*(t23*y1 + t6*x1) + t17*(t20*y1 + t7*x1) + t21*(t10 + t9);
		jacob(0, 3) +=  t11*t15 + t16*(t23*t4 + t5*t6) + t17*(t20*t4 + t5*t7) + t21*(t1*t5 + t4*t7);
		jacob(0, 4) +=  t17*y1;
		jacob(0, 5) +=  t21*y1;
		jacob(1, 0) +=  t17;
		jacob(1, 1) +=  2;
		jacob(2, 0) +=  t21;
		jacob(2, 2) +=  2;
		jacob(3, 0) +=  t11*t15 + t11*(t20*y1 + t7*x1) + t16*(t23*t4 + t5*t6) + t22*(t10 + t9);
		jacob(3, 1) +=  t11;
		jacob(3, 2) +=  t22;
		jacob(3, 3) +=  t11*(t20*t4 + t5*t7) + t15*(t3*t4*x1 + t6*pow(y1, 3)) + t16*(t23*pow(y1, 3) + t4*t6*x1) + t22*(t1*t5 + t4*t7);
		jacob(3, 4) +=  t11*y1;
		jacob(3, 5) +=  t22*y1;
		jacob(4, 0) +=  y1*(t20*y1 + t7*x1);
		jacob(4, 3) +=  y1*(t20*t4 + t5*t7);
		jacob(5, 0) +=  y1*(t10 + t9);
		jacob(5, 2) +=  t24;
		jacob(5, 3) +=  y1*(t1*t5 + t4*t7);
		jacob(5, 5) +=  t4;

			fVector(0) +=  t15*t21 + t16*t17;
		fVector(1) +=  2*dx0 + 2*dx1*y1 + t18 + t19 - 2*x2;
		fVector(2) +=  2*dy0 + 2*dy1*y1 + t23*x1 + t8*y1 - 2*y2;
		fVector(3) +=  t11*t16 + t15*t22;
		fVector(4) +=  t16*y1;
		fVector(5) +=  t15*y1;
...
void JelloComplex1::CalcJelloTransform(Mat img1, Mat img2){
	params = new float[NUM_PARAMS];
...
void JelloComplex1::CreateAbsoluteTransform(JelloComplex1 prevTransform){
	for(int row=0;row<imgHeight;row++){
...
void JelloComplex1::TransformPoint(float x, float y, float &x2, float &y2){
	EXTRACT_PARAMS(params)
...
void JelloComplex1::TransformPointAbs(float x, float y, float &x2, float &y2){
	int ix = round(x);
	int iy = round(y);
